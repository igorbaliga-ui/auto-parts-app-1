...
const Header = () => {
  ...
  // Событие 'beforeinstallprompt' иногда приходит от браузера с задержкой —
  // не сразу при загрузке страницы, а через некоторое время после начала
  // взаимодействия с сайтом. Если пользователь успел нажать на иконку раньше
  // этого момента, открылась инструкция вместо прямой установки. Как только
  // событие всё же приходит, пока инструкция для Android ещё открыта —
  // закрываем её и сразу вызываем нативное окно установки, без повторного клика.
  useEffect(() => {
    if (guideOpen && guideTab === "android" && canInstall) {
      setGuideOpen(false);
      promptInstall();
    }
  }, [canInstall, guideOpen, guideTab, promptInstall]);

...
const LEAD_ERRORS_URL = 'https://functions.poehali.dev/1a8b7b50-021b-4933-8b36-5e3cf4dc94e9';
...
type AdminErrorsTabProps = {
  adminPassword: string;
};

/**
 * Журнал сбоев при отправке заявки клиентом (backend leads-submit падает с исключением —
 * например, ошибка БД). Дублирует push-уведомление менеджеру постоянным списком, чтобы
 * случаи не терялись, если push пропущен/отключён. Позволяет отметить случай отработанным.
 */
const AdminErrorsTab = ({ adminPassword }: AdminErrorsTabProps) => {
  const [errors, setErrors] = useState<LeadSubmitError[]>([]);
  ...
  const resolve = async (id: number) => {
    setResolvingId(id);
    try {
      const res = await fetch(LEAD_ERRORS_URL, {
        method: 'POST',
      ...
      <div className="flex items-center justify-between gap-4 mb-4">
        <div>
          <h2 className="font-head uppercase tracking-wide text-lg flex items-center gap-2">
            ...
            Ошибки отправки заявок
          </h2>
          <p className="text-sm text-muted-foreground mt-1">
            Случаи, когда заявка клиента не сохранилась из-за сбоя на сервере — дублирует push-уведомление.
            {unresolvedCount > 0 && (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span>{e.phone || 'Номер неизвестен'}</span>
                </div>
                <span className="text-xs text-muted-foreground whitespace-nowrap">
                  {e.created_at ? formatDate(e.created_at) : ''}
                </span>
              </div>
              <p className="text-sm text-muted-foreground break-words">{e.error_message}</p>
              <div className="flex items-center justify-between gap-3">
                {e.resolved ? (

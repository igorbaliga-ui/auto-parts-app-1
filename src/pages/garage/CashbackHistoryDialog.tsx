...
const CashbackHistoryDialog = ({ history, open, onOpenChange }: CashbackHistoryDialogProps) => {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>История кэшбэка</DialogTitle>
          <DialogDescription>Начисления за выполненные заказы и списания</DialogDescription>
        </DialogHeader>
        {history.length === 0 ? (
            ...
            {history.map((h, i) => (
              <div
                ...
                className={`flex items-center justify-between gap-3 border-l-2 pl-3 ${
                  h.type === 'accrual' ? 'border-primary/40' : 'border-destructive/50'
                }`}
              >
                <div className="flex items-center gap-2">
                  <Icon
                    ...
                    size={14}
                    className={h.type === 'accrual' ? 'text-primary' : 'text-destructive'}
                  />
                  <div>
                ...
                >
                  {h.type === 'accrual' ? '+' : '−'}{formatMoney(h.amount)}
                </span>
              </div>
            ))}
          </div>
